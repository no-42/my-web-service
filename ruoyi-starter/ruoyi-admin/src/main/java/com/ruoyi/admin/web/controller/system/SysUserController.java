...
public class SysUserController extends BaseController {
    ...
    public AjaxResult getInfo(@PathVariable(value = "userId", required = false) String userId) {
        ...
        ajax.put("roles", user != null && user.isAdmin() ? roles : roles.stream().filter(r -> !r.isAdmin()).collect(Collectors.toList()));
        ajax.put("posts", postService.selectPostAll());
        if (StringUtils.isNotNull(userId)) {
            SysUserEntity sysUserEntity = userService.selectUserById(userId);
            ajax.put(AjaxResult.DATA_TAG, sysUserEntity);
            ajax.put("postIds", postService.selectPostListByUserId(userId));
            ajax.put("roleIds", sysUserEntity.getRoles().stream().map(SysRoleEntity::getId).collect(Collectors.toList()));
        }
        return ajax;
    }
    ...
    @PostMapping
    public AjaxResult add(@Validated @RequestBody SysUserEntity user) {
    ...
    @PutMapping
    public AjaxResult edit(@Validated @RequestBody SysUserEntity user) {
