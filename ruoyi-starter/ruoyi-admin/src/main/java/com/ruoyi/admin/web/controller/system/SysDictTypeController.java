...
public class SysDictTypeController extends BaseController {
    ...
    @PostMapping
    public AjaxResult add(@Validated @RequestBody SysDictTypeEntity dict) {
        if (UserConstants.NOT_UNIQUE.equals(dictTypeService.checkDictTypeUnique(dict))) {
            return AjaxResult.error("新增字典'" + dict.getName() + "'失败，字典类型已存在");
        }
    ...
    @PutMapping
    public AjaxResult edit(@Validated @RequestBody SysDictTypeEntity dict) {
        if (UserConstants.NOT_UNIQUE.equals(dictTypeService.checkDictTypeUnique(dict))) {
            return AjaxResult.error("修改字典'" + dict.getName() + "'失败，字典类型已存在");
        }
