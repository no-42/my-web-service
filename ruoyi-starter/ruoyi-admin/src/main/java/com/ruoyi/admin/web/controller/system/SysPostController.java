...
public class SysPostController extends BaseController {
    ...
    @PostMapping
    public AjaxResult add(@Validated @RequestBody SysPostEntity post) {
        if (UserConstants.NOT_UNIQUE.equals(postService.checkPostNameUnique(post))) {
            return AjaxResult.error("新增岗位'" + post.getName() + "'失败，岗位名称已存在");
        } else if (UserConstants.NOT_UNIQUE.equals(postService.checkPostCodeUnique(post))) {
            return AjaxResult.error("新增岗位'" + post.getName() + "'失败，岗位编码已存在");
        }
    ...
    @PutMapping
    public AjaxResult edit(@Validated @RequestBody SysPostEntity post) {
        if (UserConstants.NOT_UNIQUE.equals(postService.checkPostNameUnique(post))) {
            return AjaxResult.error("修改岗位'" + post.getName() + "'失败，岗位名称已存在");
        } else if (UserConstants.NOT_UNIQUE.equals(postService.checkPostCodeUnique(post))) {
            return AjaxResult.error("修改岗位'" + post.getName() + "'失败，岗位编码已存在");
        }
